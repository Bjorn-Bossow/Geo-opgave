function [G_r, G_theta, G_lambda] = design_SHA(r, theta, lambda, N, varargin);
% [G_r, G_theta, G_lambda] = design_SHA(r, theta, lambda, N)
%
...
% and the magnetic component B_i, where "i" is "r", "theta" or "lambda":
%        B_i = A_i*x
...
%
% [G_r, G_theta, G_lambda] = design_SHA(r, theta, lambda, N, i_e_flag)
...
max_size = max([size(r); size(theta); size(lambda)]); 
...
if length(lambda)   == 1; lambda = lambda*ones(max_size); end;
% check for equal length of all input parameters
if size(r) ~= size(theta) | size(r) ~= size(lambda);
...
lambda = reshape(lambda, max_length, 1);
...
if exist('design_SHA_m') == 3
    [G_r, G_theta, G_lambda] = design_SHA_m(r, theta, lambda, N, i_e_flag);
else
     [G_r, G_theta, G_lambda] = design_SHA_matlab(r, theta, lambda, N, i_e_flag);
end

% -----------------------------------------------------------------------

function [G_r, G_theta, G_lambda] = design_SHA_matlab(r, theta, lambda, N, varargin);
% [G_r, G_theta, G_lambda] = design_SHA_matlab(r, theta, lambda, N)
%
% Calculates design matrices A_i that connects the vector 
% of (Schmidt-normalized) spherical harmonic expansion coefficients, 
% x = (g_1^0; g_1^1; h_1^1; g_2^0; g_2^1; h_2^1; ... g_N^N; h_N^N) 
...
G_r       = zeros(N_data, N_koeff);
G_theta   = zeros(N_data, N_koeff);
G_lambda     = zeros(N_data, N_koeff);
...
for n = 1:N
   ...
   if ~strcmp(i_e_flag, 'ext') % internal sources by default
      for m = 0:n;  
         cos_lambda   = cos(m*lambda);
         sin_lambda   = sin(m*lambda);
         
         if m == 0
            k = k+1;       % terms corresponding to g_n^0
            G_r(:,k)       =  (n+1).*r_n(:).*Pnm(:,1);
            G_theta(:,k)   = -r_n(:).*dPnm(:,1);
            G_lambda(:,k)     =  r_n(:)*0;
         else
            k = k+1;       % terms corresponding to g_n^m
            G_r(:,k)       =  (n+1).*r_n(:).*cos_lambda.*Pnm(:,m+1);
            G_theta(:,k)   = -r_n(:).*cos_lambda.*dPnm(:,m+1);
            G_lambda(:,k)     =  r_n(:).*m.*sin_lambda.*Pnm(:,m+1)./sin_theta;
            
            k = k+1;       % terms corresponding to h_n^m
            G_r(:,k)       =  (n+1).*r_n(:).*sin_lambda.*Pnm(:,m+1);
            G_theta(:,k)   = -r_n(:).*sin_lambda.*dPnm(:,m+1);
            G_lambda(:,k)     = -r_n(:).*m.*cos_lambda.*Pnm(:,m+1)./sin_theta;
         end    
      end; % m
      ...
      for m = 0:n;  
         ...
         if m == 0
            ...
            G_r(:,k)       = -n.*r_n(:).*cos_lambda.*Pnm(:,m+1);
            G_theta(:,k)   = -r_n(:).*cos_lambda.*dPnm(:,m+1);
            G_lambda(:,k)     =  r_n(:).*m.*sin_lambda.*Pnm(:,m+1)./sin_theta;
            
            k = k+1;       % terms corresponding to s_n^m
            G_r(:,k)       = -n.*r_n(:).*sin_lambda.*Pnm(:,m+1);
            G_theta(:,k)   = -r_n(:).*sin_lambda.*dPnm(:,m+1);
            G_lambda(:,k)     = -r_n(:).*m.*cos_lambda.*Pnm(:,m+1)./sin_theta;
         end    
      end; % m
